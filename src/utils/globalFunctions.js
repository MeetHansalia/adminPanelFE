...
export const actionConfirmWithLoaderAlert = (
  customData = {},
  options = {},
  callback = null
) => {
  Swal?.fire({
    title:
      customData?.title || (customData?.title === null ? "" : "Are you sure?"),
    text:
      customData?.text ||
      (customData?.text === null
        ? ""
        : `You want to ${(customData && customData?.action) || "delete"} ${
            (customData && customData?.moduleName) || "this"
          }?`),
    icon: "warning",
    showCancelButton: true,
    confirmButtonText: `Yes, ${
      (customData && customData?.action) || "delete"
    } it!`,
    customClass: {
      confirmButton: "btn bg-primary",
      ...
      return axiosApiCall({
        ...
        .catch((error) => {
          ...
          Swal?.showValidationMessage(
            `<span class="text-danger">Request failed: ${errorMessage}</span>`
          );
        });
    },
    allowOutsideClick: () => !Swal?.isLoading(),
...
export const getPanelName = (pathname) => {
  ...
  const isValidPanel = Object.values(USER_PANELS).some(
...
export const isUserHasPermission = (fnInput = null) => {
  ...
  const mainPermissionStatus = permissions?.some(
    (obj) => obj?.permission === permissionToCheck
  );
  ...
  const matchedPermissionObj = permissions?.find(
    (obj) => obj?.permission === permissionToCheck
  );
...
export const isUserHasPermissions = (fnInput = null) => {
  ...
  for (let index = 0; index < permissionsToCheck?.length; index++) {
    ...
    const subPermissionsToCheck =
      permissionToCheckElement?.subPermissions || [];
...
export const getFullName = (fnInput = null) => {
  return titleize(
    (fnInput?.first_name || "") + " " + (fnInput?.last_name || "")
  );
};
...
export const convertUrlToDocumentedPath = (fnInput = null) => {
  ...
  Object.entries(params).forEach(([key, value]) => {
    ...
    documentedPath = documentedPath.replace(
